...
void init_input_data(InputDataType& data, size_t seed, size_t numDataPoints) {
  ...
  data.particles.r = (FTYPE*) aligned_alloc(32, sizeof(FTYPE) * numDataPoints);     //cambio
  data.particles.phi = (FTYPE*) aligned_alloc(32, sizeof(FTYPE) * numDataPoints);   //cambio
  ...
  for (size_t i = 0; i < numDataPoints; ++i) {
    data.particles.r[i] = distr(generator);       //cambio
    data.particles.phi[i] = distphi(generator);   //cambio
  }
}

void free_input_data(InputDataType& data) {   //cambio
  free(data.particles.r);
  free(data.particles.phi);
  data.particles.r = nullptr;
  data.particles.phi = nullptr;
